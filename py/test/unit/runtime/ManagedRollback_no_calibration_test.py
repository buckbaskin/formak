...
import numpy as np
from formak.runtime import ManagedRollback, StampedReading
...
def test_constructor():
    ...
    _mr = ManagedRollback(ekf=ekf, start_time=0.0, state=state, covariance=covariance)
...
def test_tick_empty_readings(dt):
    ...
    control = ekf.Control(control_velocity=-1.0)
    state0p1 = mr.tick(start_time + dt, control=control, readings=[])
...
def test_tick_multi_reading(output_dt, shuffle_order):
    ...
    state0p1 = mr.tick(start_time + output_dt, control=control, readings=readings)
