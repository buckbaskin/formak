...
def cc_formak_model(name, pymain, pysrcs, pydeps = None, python_version = None, ccdeps = None, visibility = None, **kwargs):
    ...
    native.genrule(
        name = GENRULE_NAME,
        srcs = [PY_BINARY_NAME] + MODEL_TEMPLATES,
        outs = OUTPUT_FILES,
        cmd = "python3 $(location " + pymain + ") --headertemplate $(location " + MODEL_TEMPLATES_HEADER + ") --sourcetemplate $(location " + MODEL_TEMPLATES_SOURCE + ") --header $(location generated/jinja_basic_class.h) --source $(location generated/jinja_basic_class.cpp)",
        tools = [pymain],
        visibility = ["//visibility:private"],
    )
