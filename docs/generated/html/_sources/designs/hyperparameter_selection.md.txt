...
The organization of the fititng structure via state machine could be thought of
...
answer. Under the hood, a pipeline may ultimately become the answer.

The two seem similar, but they are orthogonal axis. The state machine is an
organizational tool for creating the estimator, the pipeline is for fitting.

### 2023-11-12

#### `formak.python` UI change

Previously, the scikit-learn interface was mixed with the Kalman Filter
...
`SklearnEKFAdapter` that inherits from scikit-learn's `BaseEstimator`.
